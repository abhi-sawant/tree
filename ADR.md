...
offline *from*. Cloud sync and accounts would reopen this and are excluded permanently — not
...
entire global pool, never just the open tree, because a file export is a *copy of the data*, not a
view of it. Phase 6 added the family book as a deliberate, named exception: it is scoped to the open
tree because a book is a document about *one* family, not a data copy — the same reasoning that puts
statistics and the health validator's tree-membership check on opposite sides of the same question.

### D15 — Desktop-first
**Status:** Accepted; still the case through Phase 7. `FUTURE-SCOPE.md §10` scopes the mobile pass
that would revisit this.
...
**Decision:** A `PartialDate` denotes a *span* ("1950" is the whole year). Use `definitelyBefore` /
...
*before* commit (a listener must be idempotent — an aborted write can still signal), and excludes
...
### D30 — Privacy redaction inverts the validator rule: undecidable resolves to *redact*
...
*name* (no UUIDs in a spreadsheet). It cannot carry photos, custom fields, marriage dates, or
...
-style automated audit nor high-contrast palette values exist yet — see `FUTURE-SCOPE.md` for the
mobile-interaction pass this would likely ship alongside.
