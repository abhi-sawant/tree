...
function person(overrides: Partial<Person> = {}): Person {
  const now = Date.now()
  return { id: id(), givenName: "Test", createdAt: now, updatedAt: now, ...overrides }
}
...
describe("toReactFlowGraph", () => {
  it("maps a person node to type 'person', sized to match the ELK constants, carrying the Person in data", () => {
    ...
    const graph = toElkGraph({ people: [a], relationships: [], treeMembers: [member(treeId, a.id)] })

    const { nodes } = toReactFlowGraph({ graph, positions: { [`person:${a.id}`]: { x: 1, y: 2 } }, people: [a] })
  ...
  it("maps a union node to type 'union', sized to match the ELK constants", () => {
    ...
    const b = person()
    const child = person()
    const relationships = [parentChild(a.id, child.id), parentChild(b.id, child.id), spouse(a.id, b.id)]
    const graph = toElkGraph({
      people: [a, b, child],
      relationships,
      treeMembers: [member(treeId, a.id), member(treeId, b.id), member(treeId, child.id)],
    })

    const unionId = graph.children!.map((n) => n.id).find((nid) => nid.startsWith("union:"))!
  ...
  it("maps edges from the ELK edge's sources[0]/targets[0]", () => {
    ...
    const { edges } = toReactFlowGraph({ graph, positions: {}, people: [parent, child] })
  ...
  it("produces correct node/edge counts for a remarriage fixture", () => {
    ...
    const { nodes, edges } = toReactFlowGraph({ graph, positions: {}, people })
