...
export function PersonNode({ data }: NodeProps<PersonNodeType>) {
  const { person } = data
  const name = [person.givenName, person.familyName].filter(Boolean).join(" ") || "Unnamed"
  const dates = [formatPartialDate(person.birth), formatPartialDate(person.death)]
    .filter(Boolean)
    .join(" – ")

  return (
    <div className="flex h-full w-full items-center gap-2 rounded-md border border-border bg-card px-3 py-2 shadow-sm">
